...
class TempRole(commands.Cog):
    ...
    async def temprole(self, ctx):
        ...
        embed = nextcord.Embed(
            ...
            description=f"Added temporary role <@&{role.id}> to {user.mention} for **{readable}**.", 
    ...
    async def temprole_slash_add(self, interaction: nextcord.Interaction, user: nextcord.Member, duration: str, role: nextcord.Role):
        ...
        embed = nextcord.Embed(title=f"Temporary Role Case #{current_case}", description=f"Added temporary role <@&{role.id}> to {user.mention} for {readable}.", color=nextcord.Color.blurple())
