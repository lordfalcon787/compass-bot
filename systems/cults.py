...
class Cults(commands.Cog):
    ...
    async def on_ready(self):
        ...
        collection.insert_one({"_id": "cult_points"})
        collection.insert_one({"_id": "cult_list"})
    ...
    async def create(self, interaction: nextcord.Interaction, cult_name: str):
        ...
        if cult_name in collection.find_one({"_id": "cult_list"}):
            await interaction.response.send_message("That cult already exists.", ephemeral=True)
            return
        collection.update_one({"_id": "cult_list"}, {"$set": {cult_name: {"members": []}}}, upsert=True)
        await interaction.response.send_message(f"Cult {cult_name} created successfully.", ephemeral=True)
    ...
    async def assign(self, interaction: nextcord.Interaction, user: nextcord.Member, cult: str = SlashOption(description="The cult to assign the user to.", autocomplete=True)):
        ...
        if cult not in doc:
            await interaction.response.send_message("That cult does not exist.", ephemeral=True)
            return
        if user.id in doc[cult]["members"]:
            await interaction.response.send_message("That user is already in that cult.", ephemeral=True)
            return
        ...
        if user.id in all_members:
            await interaction.response.send_message("That user is already in a cult, please remove them from that cult first.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"User {user.mention} assigned to cult {cult}.", ephemeral=True)
    ...
    async def deassign(self, interaction: nextcord.Interaction, user: nextcord.Member, cult: str = SlashOption(description="The cult to remove the user from.", autocomplete=True)):
        ...
        if cult not in doc:
            if cult not in doc2[str(user.id)]["cult"]:
                ...
                await interaction.response.send_message(f"User {user.mention} removed from cult {cult}.", ephemeral=True)
                return
        try:
    ...
    async def delete(self, interaction: nextcord.Interaction, cult: str = SlashOption(description="The cult to delete.", autocomplete=True)):
        ...
        await interaction.response.send_message(f"Cult {cult} deleted successfully.", ephemeral=True)
    ...
    async def setowner(self, interaction: nextcord.Interaction, user: nextcord.Member, cult: str = SlashOption(description="The cult to set the owner of.", autocomplete=True)):
        ...
        if user.id not in doc[cult]["members"]:
            await interaction.response.send_message("That user is not in that cult, please assign them to the cult first.", ephemeral=True)
            return
        collection.update_one({"_id": "cult_list"}, {"$set": {f"{cult}.owner": user.id}})
        await interaction.response.send_message(f"Owner of cult {cult} set to {user.mention}.", ephemeral=True)
    ...
    async def remove(self, interaction: nextcord.Interaction, user: nextcord.Member, amount: int):
        ...
        if user not in interaction.guild.members:
            ...
            return
        collection.update_one({"_id": "cult_points"}, {"$inc": {f"{user.id}.points": -amount}})
        await interaction.response.send_message(f"Removed {amount} points from {user.mention}.", ephemeral=True)
