...
class GitHubUploader:
    ...
    async def upload_file(self, file_content: bytes, file_path: str, commit_message: Optional[str] = None) -> str:
        ...
        Args:
            ...
            file_path: Path where to store the file in the repo (e.g., "transcripts/file.html")
            commit_message: Optional commit message
            
        Returns:
            The raw GitHub URL to access the file
        """
    ...
    async def upload_transcript(self, transcript_content: bytes, channel_name: str, timestamp: datetime) -> str:
        """
        Upload a Discord transcript file to GitHub.
        
        Args:
            transcript_content: The HTML transcript content as bytes
            channel_name: Name of the Discord channel
            timestamp: When the transcript was generated
            
        Returns:
            The GitHub URL to access the transcript
        """
        # Create filename with timestamp
        filename = f"transcript-{channel_name}-{timestamp.strftime('%Y%m%d-%H%M%S')}.html"
        file_path = f"transcripts/{filename}"
        commit_message = f"Add transcript for #{channel_name} - {timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
        
        return await self.upload_file(transcript_content, file_path, commit_message)

# Helper function to get GitHub uploader instance
def get_github_uploader() -> Optional[GitHubUploader]:
    """
    Get GitHub uploader instance from config.json file.
