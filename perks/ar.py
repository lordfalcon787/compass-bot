    ...
    async def ar(self, interaction: nextcord.Interaction,
        ...
        if word is None and not any(role.id in allowed_roles for role in interaction.user.roles):
            ...
            if interaction.user.nick:
                nick = interaction.user.nick.lower()
            if word.lower() not in nick and word.lower() not in interaction.user.name.lower() and word.lower() not in interaction.user.display_name.lower():
        ...
        if TYPE == 1:
            ...
            current_words = self.cache.get(str(interaction.guild.id))
            if current_words:
                current_words.append(word)
                self.cache[str(interaction.guild.id)] = current_words
            else:
                self.cache[str(interaction.guild.id)] = [word]
